...
import { Heart, Trash2, Trophy, HelpCircle, CheckSquare, MessageSquare, Send } from 'lucide-react'
  ...
  return (
    <div className="p-4 sm:p-8 max-w-2xl">
      <div className="mb-8">
        ...
        <p className="text-[#555] text-sm">Share wins, ask questions, and hold each other accountable.</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-16">
          ...
          <p className="text-[#444] text-sm">No posts yet. Be the first to post.</p>
        </div>
      ) : (
        <div className="space-y-3">
          {filtered.map(post => {
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <span className={cn('text-sm font-medium', isMe ? 'text-[#C64E3A]' : 'text-[#F5F1ED]')}>
                        {post.user_name}{isMe && <span className="text-[#444] font-normal text-xs ml-1">(you)</span>}
                      </span>
                      <span className="text-[#444] text-xs ml-2">{timeAgo(post.created_at)}</span>
                    </div>
                  </div>
                  <span className={cn('text-[10px] font-semibold uppercase tracking-wide px-2 py-0.5 rounded-full', TYPE_BADGE[post.post_type])}>
